...
const outDir = path.join(root, 'outputs', 'surfit_explainer');
...
function serveStatic(port = 8787) {
  const server = createServer(async (req, res) => {
    try {
      let reqPath = (req.url || '/').split('?')[0];
      if (reqPath === '/') reqPath = '/surfit-explainer/index.html';
...
async function main() {
  ...
  const server = await serveStatic(8787);
  ...
  try {
    ...
    await page.goto('http://127.0.0.1:8787/surfit-explainer/index.html', { waitUntil: 'networkidle' });
    ...
    const outFile = path.join(outDir, 'surfit-explainer-54s.webm');
