...
.stTabs [data-baseweb="tab"] { background: transparent !important; color: #7a9ab8 !important; font-family: 'DM Sans', sans-serif !important; font-size: 11px !important; letter-spacing: 0.12em !important; text-transform: uppercase !important; padding: 12px 28px !important; border: none !important; border-bottom: 2px solid transparent !important; border-radius: 0 !important; }
...
.stSelectbox > div > div:hover { border-color: #26c0ff !important; }
...
[data-testid="stSliderTrackFill"] { background: #26c0ff !important; }
[data-testid="stSliderThumb"] { background: #26c0ff !important; border: none !important; }
.stButton > button { background: #26c0ff !important; color: #0b1220 !important; font-family: 'DM Sans', sans-serif !important; font-weight: 600 !important; font-size: 11px !important; letter-spacing: 0.14em !important; text-transform: uppercase !important; border: none !important; border-radius: 8px !important; padding: 14px 24px !important; }
.stButton > button:hover { background: #4dcdff !important; box-shadow: 0 4px 24px rgba(38,192,255,0.35) !important; }
...
with hc1:
    st.markdown(
        f'<div style="display:flex;align-items:center;gap:14px;padding:20px 0 12px;">'
        f'{WAVE}'
        f'<div><div style="font-family:Righteous,cursive;font-size:22px;line-height:1;display:flex;align-items:baseline;">'
        f'<span style="color:#26c0ff;">SURFIT</span>'
        f'<span style="color:#7a9ab8;font-size:15px;margin:0 2px;">.</span>'
        f'<span style="color:#ff731e;">AI</span>'
        f'</div><div style="font-size:9px;letter-spacing:0.15em;text-transform:uppercase;color:#7a9ab8;margin-top:3px;">SAW Platform</div>'
...
with tab1:
    ...
    with col2:
        if run_button:
            ...
            ctx  = RunContext(run_id=str(uuid.uuid4()), saw_id=spec["saw_id"], state={"_approval_granted": approval_granted, "_approval_wait_ms": wait_ms})
...
with tab2:
    ...
    if history is not None and not history.empty:
        ...
        st.markdown('<div class="sf-empty"><div class="sf-empty-text">No runs yet — go to Run SAW and fire one off</div></div>', unsafe_allow_html=True)
